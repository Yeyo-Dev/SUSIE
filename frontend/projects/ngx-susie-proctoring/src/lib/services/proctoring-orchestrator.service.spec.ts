...
describe('ProctoringOrchestratorService Recovery', () => {
  ...
  // ══════════════════════════════════════════════════════════════════════════════
  // REQ-PO-006: handleViolation() with NETWORK_TIMEOUT
  // ══════════════════════════════════════════════════════════════════════════════

  describe('handleViolation() NETWORK_TIMEOUT', () => {
    it('should accept NETWORK_TIMEOUT type without type casting', fakeAsync(() => {
      // GIVEN: Orchestrator in MONITORING state
      const config = createSusieConfig();
      const { callbacks } = createCallbacks();
      service.initialize(config, callbacks);
      (service as any).state.set('MONITORING');

      // WHEN: A NETWORK_TIMEOUT violation is handled
      ...
      // THEN: handleViolation should accept NETWORK_TIMEOUT directly (no 'as any' needed)
      service.handleViolation(violation);

      // AND: totalViolations is incremented
      expect((service as any).totalViolations()).toBe(1);

      // AND: onViolation callback is called
      expect(callbacks.onViolation).toHaveBeenCalledWith(violation);

      // AND: sendEvent is called with correct trigger mapping
      expect(mocks.evidenceService.sendEvent).toHaveBeenCalledWith(
        jasmine.objectContaining({
          type: 'BROWSER_EVENT',
          trigger: 'NETWORK_TIMEOUT',
          browser_focus: jasmine.any(Boolean),
        })
      );
    ...
    it('should increment totalViolations but not trigger tab switch logic for NETWORK_TIMEOUT', fakeAsync(() => {
      // GIVEN: Orchestrator in MONITORING state
      const config = createSusieConfig();
      const { callbacks } = createCallbacks();
      service.initialize(config, callbacks);
      ...
      // WHEN: NETWORK_TIMEOUT violation is handled
      service.handleViolation({
        type: 'NETWORK_TIMEOUT',
        message: 'Network timeout',
        timestamp: new Date().toISOString(),
      });

      // THEN: totalViolations is incremented
      expect((service as any).totalViolations()).toBe(1);

      // AND: tabSwitchCount is NOT incremented (NETWORK_TIMEOUT is not a tab switch)
      expect((service as any).tabSwitchCount()).toBe(0);

      discardPeriodicTasks();
    }));

    it('should call onViolation callback with NETWORK_TIMEOUT violation', fakeAsync(() => {
      // GIVEN: Orchestrator in MONITORING state
      const config = createSusieConfig();
      const { spies } = createCallbacks();
      service.initialize(config, spies);
      (service as any).state.set('MONITORING');
      ...
      // WHEN: handleViolation is called
      service.handleViolation(violation);

      // THEN: onViolation is called with the NETWORK_TIMEOUT violation
      expect(spies.onViolation).toHaveBeenCalledWith(violation);
