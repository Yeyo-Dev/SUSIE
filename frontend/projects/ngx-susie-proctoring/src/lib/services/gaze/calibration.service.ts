...
export class CalibrationService {
    ...
    complete(currentState: () => GazeState): GazeCalibrationMetrics {
        ...
        this.loggerService.log(`CALIBRACIÓN COMPLETA | Clicks: ${metrics.calibrationClicks} | Frames: ${metrics.calibrationFrames} | Duración: ${duration}s | ${quality}`);

        if (this.webgazerRef) {
            try {
                if (typeof this.webgazerRef.resume === 'function') {
                    this.webgazerRef.resume();
                    this.loggerService.log('webgazer.resume() llamado');
                }
            } catch (e) {
