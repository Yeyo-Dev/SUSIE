...
export class DomManagerService {
    ...
    injectStyles(): void {
        ...
        this.loggerService.log('Estilos preventivos inyectados (Ghost Mode)');
    }
    ...
    configureVideoForCalibration(): void {
        ...
        if (videoFeed?.paused) {
            videoFeed.play().catch(e => this.loggerService.warn('Error al hacer play:', e));
        }

        this.loggerService.log('Cámara configurada para calibración (semi-transparente, detrás de puntos)');
    }
    ...
    showGazeDot(): void {
        ...
        this.loggerService.log('Punto de gaze visible');
    }
