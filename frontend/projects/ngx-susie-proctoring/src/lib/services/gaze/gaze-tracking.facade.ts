...
export class GazeTrackingFacadeService {
    // ─── Señales Públicas (misma API que el monolito) ─────────────────────────────
    ...
    // Delegado de FaceDetectionService
    ...
    // Delegado de DeviationDetectionService
    ...
    constructor() {
        (window as any).gazeService = this; // ayuda para debug
    ...
    configure(
        ...
        this.faceDetection.configure(this.logger, onNoFace, () => {
            // Rostro recuperado → resetear filtros de Kalman
    ...
    async startCalibration(existingStream?: MediaStream | null): Promise<boolean> {
        if (this.isInitializing()) {
            this.loggerService.log('Ya inicializando, ignorando.');
            return false;
        }
        ...
        try {
            ...
            this.loggerService.log('Iniciando calibración...');

            this.domManager.injectStyles();
            this.domManager.startAggressiveMuting();

            // Conectar listeners del bridge
            ...
            // Actualizar servicio de calibración con referencia a webgazer
    ...
    private onRawGaze(event: RawGazeEvent): void {
        ...
        // Análisis de pose de cabeza (solo durante tracking)
        if (this.gazeState() === 'TRACKING') {
        ...
        // Suavizado de señal (pasar confidence al filtro Kalman)
        ...
        // Solo almacenar en estado TRACKING (no durante calibración)
        if (this.gazeState() === 'TRACKING') {
...
// Re-exportar como GazeTrackingService para compatibilidad hacia atrás
