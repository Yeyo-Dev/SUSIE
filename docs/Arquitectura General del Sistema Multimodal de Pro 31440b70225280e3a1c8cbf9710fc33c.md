...
### Nuevo diagrama de Red Bayesiana

```mermaid
graph TD
    %% Nodos Base
    V["Visión YOLOv8<br>V ∈ {Normal, Ausente, Objeto_Prohibido, Multitud}"]
    A["Audio NLP<br>A ∈ {Silencio, Neutral, Doméstico, Sospechoso}"]
    G["Gaze Tracking<br>G ∈ {Concentrado, Fuera_de_Pantalla, Atención_Secundaria, Errático}"]
    F(("Fraude Académico<br>P(F | V, A, G)"))

    %% Dependencias Directas hacia el Fraude
    V ==>|"Evidencia Primaria"| F
    A ==>|"Contexto Semántico"| F
    G ==>|"Comportamiento"| F

    %% Relaciones Condicionales (Sinergias y Dependencias)
    V -.->|"Condiciona Fiabilidad<br>Si V=Ausente, G pierde peso"| G
    G -.->|"Sinergia de Intención<br>Modula Falsos Positivos"| A
...
flowchart TD
    subgraph FE["🖥️ Frontend (Angular)"]
        CAM["📷 Captura Webcam / Pantalla"]
        MIC["🎙️ Captura Audio"]
        WS_CLIENT["WebSocket Client (retroalimentación en tiempo real)"]
        SESION_BTN["Botón Finalizar Sesión"]
    end

    subgraph BE["⚙️ Backend (Fastify / TypeScript)"]
        REST_SNAP["POST /monitoreo/evidencias\n(snapshot)"]
        REST_AUD["POST /monitoreo/evidencias\n(audio)"]
        REST_SESION["PUT /sesiones/:id_sesion/finalizar"]
        PRODUCER["ProducerService\nExchange: proctoring_events (topic)"]
        WS_SERVER["WebSocket Server\n(envía alertas al frontend)"]

        subgraph CONSUMER_INFRAC["Consumer: q_infracciones"]
            CONS_INF["Recibe infracciones\ndel Worker IA"]
            REDIS_SAVE["Guarda en Redis\n(sesion_id → infracciones[])"]
            WS_PUSH["Notifica via WebSocket\nal Frontend"]
        end

        subgraph CONSUMER_SESION["Al finalizar sesión"]
            REDIS_READ["Lee todas las evidencias\nde Redis (sesion_id)"]
            PUB_EVID["Publica en q_evidencia\n(batch de evidencias)"]
        end
    end

    subgraph MQ["🐇 RabbitMQ — Exchange: proctoring_events (topic)"]
        Q_SNAP["📸 q_snapshots\nrouting: stream.snapshot"]
        Q_AUD["🎵 q_audios\nrouting: stream.audio"]
        Q_INFRAC["🚨 q_infracciones\n(resultados en tiempo real)"]
        Q_EVID["📦 q_evidencia\n(cierre de sesión → inferencia)"]
    end

    subgraph WORKERS["🤖 Workers IA (Python + Pika)"]
        W_SNAP["Worker Snapshots\nAnálisis visual / gaze"]
        W_AUD["Worker Audio\nAnálisis de voz / ruido"]
        W_INFR["Worker Infracciones\nDetecta y clasifica infracción"]
        W_INF["Motor de Inferencia\n(evaluación final de sesión)"]
    end

    subgraph REDIS["🔴 Redis"]
        R_STORE["Hash por sesion_id\n{evidencias[], infracciones[]}"]
    end

    %% ─── Flujo de evidencias (tiempo real) ───
    CAM -->|"multipart/form-data"| REST_SNAP
    MIC -->|"multipart/form-data"| REST_AUD
    REST_SNAP --> PRODUCER
    REST_AUD --> PRODUCER
    PRODUCER -->|"stream.snapshot"| Q_SNAP
    PRODUCER -->|"stream.audio"| Q_AUD

    %% Workers consumen y analizan
    Q_SNAP --> W_SNAP
    Q_AUD --> W_AUD
    W_SNAP --> W_INFR
    W_AUD --> W_INFR

    %% Workers devuelven infraccion a q_infracciones
    W_INFR -->|"reply_to: q_infracciones"| Q_INFRAC

    %% Backend consume q_infracciones
    Q_INFRAC --> CONS_INF
    CONS_INF --> REDIS_SAVE
    REDIS_SAVE <--> R_STORE
    CONS_INF --> WS_PUSH
    WS_PUSH --> WS_SERVER
    WS_SERVER -->|"WebSocket alert"| WS_CLIENT

    %% ─── Flujo de cierre de sesión ───
    SESION_BTN -->|"PUT /sesiones/:id/finalizar"| REST_SESION
    REST_SESION --> REDIS_READ
    REDIS_READ <-->|"HGETALL sesion_id"| R_STORE
    REDIS_READ --> PUB_EVID
    PUB_EVID -->|"routing: session.close"| Q_EVID
    Q_EVID --> W_INF
    W_INF -->|"Resultado final guardado\nen DB o S3"| BE

    %% Estilos
    classDef queue fill:#f0a500,color:#000,stroke:#c07800,rx:6
    classDef worker fill:#4a90d9,color:#fff,stroke:#2060a0,rx:6
    classDef backend fill:#2d6a4f,color:#fff,stroke:#1b4332,rx:6
    classDef frontend fill:#6b3fa0,color:#fff,stroke:#4a2070,rx:6
    classDef redis fill:#cc2200,color:#fff,stroke:#991100,rx:6

    class Q_SNAP,Q_AUD,Q_INFRAC,Q_EVID queue
    class W_SNAP,W_AUD,W_INFR,W_INF worker
    class REST_SNAP,REST_AUD,REST_SESION,PRODUCER,WS_SERVER,CONS_INF,REDIS_SAVE,WS_PUSH,REDIS_READ,PUB_EVID backend
    class CAM,MIC,WS_CLIENT,SESION_BTN frontend
    class R_STORE redis
```

### SUSIE EVALUATION (diseño preliminar, solo para asegurar compatibilidad en un futuro)

```mermaid
graph TD
    subgraph Plataforma Chaindrenciales
        API[API Gateway / Frontend]
    end

    subgraph Bus de Eventos - RabbitMQ
        Q_SEC[(q_close_eval<br>Respuestas Cerradas)]
        Q_TXT[(q_text_eval<br>Respuestas Abiertas)]
        Q_COD[(q_code_eval<br>Ejecución de Código)]
        
        Q_audio[(q_audio<br>Chunks de audio)]
        Q_snapshot[(q_snapshot<br>Snapshots de grabación)]
        Q_gaze[(q_gaze<br>Coordenadas de la mirada)]
    end

    API -->|Payload: Incisos| Q_SEC
    API -->|Payload: Texto del Alumno| Q_TXT
    API -->|Payload: Código Fuente| Q_COD
    API -->|Payload: Chunks de audio| Q_audio
    API -->|Payload: Snapshot| Q_snapshot
    API -->|Payload: Coordenadas| Q_gaze

    subgraph "SUSIE-PROCTORING (Módulo Principal Crítico)"
        W_snapshot[Workers de IA<br>YOLO]
        W_audio[Workers de IA<br>Whisper]
        W_gaze[Workers de IA<br>WebGaze]
        W_engine[Inference Engine<br>Bayesian Network]
    end

    subgraph "SUSIE-EVALUATION (Módulo de Calificación Aislado)"
        W_TXT[NLP Worker<br>Sentence-BERT]
        W_COD[Sandbox Worker<br>Gestor de Docker]
        W_SEC[Comparación Worker<br>Comparación de preguntas cerradas]

        subgraph "Entorno de Ejecución Segura (RCE)"
            D((Contenedor Docker<br>Efímero - Alpine))
        end
        
        W_COD -- "1. Instancia & Inyecta Código" --> D
        D -- "2. Ejecuta Unit Tests" --> D
        D -- "3. Retorna stdout (Resultados)" --> W_COD
        W_COD -. "4. Destruye Contenedor (Kill)" .-> D
    end
    ...
    W_SEC --> DB[(Base de Datos / Redis)]
    W_TXT --> DB
    W_COD --> DB

    %% Estilos de las cajas para resaltar la separación
    classDef security fill:#ffcccc,stroke:#cc0000,stroke-width:2px;
    classDef eval fill:#cce5ff,stroke:#0066cc,stroke-width:2px;
    classDef docker fill:#e6ffcc,stroke:#009900,stroke-width:2px,stroke-dasharray: 5 5;
    classDef queue fill:#ffe6cc,stroke:#d79b00,stroke-width:2px;

    class W_snapshot,W_audio,W_gaze security;
    class W_SEC,W_TXT,W_COD eval;
    class D,W_engine docker;
    class Q_SEC,Q_TXT,Q_COD,Q_snapshot,Q_gaze,Q_audio queue;
```
...
```mermaid
graph TD
    subgraph "Fase 1: Procesamiento en Tiempo Real (Hot Tier)"
        W[Workers de IA<br>Visión, Audio, Gaze]
        R[(Redis<br>Caché Volátil)]
        
        W -- "1. Escriben Logs (RPUSH)" --> R
    end

    subgraph Fase 2: Consolidación y Offloading
        IE{Motor de Inferencia<br>Red Bayesiana}
        
        R -. "2. Extrae todos los logs (LRANGE)" .-> IE
        IE -- "3. Empaqueta todo en un solo JSON" --> IE
        
        AZ[(Azure Blob Storage<br>Cold Storage)]
        PG[(PostgreSQL<br>BD Transaccional)]
        
        IE -- "4. Sube archivo JSON estático" --> AZ
        AZ -. "5. Retorna URL de Azure" .-> IE
        IE -- "6. Guarda Veredicto + URL del JSON" --> PG
    end

    subgraph "Fase 3: Consumo en Interfaz (Frontend)"
        DASH[Dashboard de Auditoría<br>Perfil Reclutador]
        
        DASH -- "7. Consulta estado del examen" --> PG
        PG -. "8. Retorna (Aprobado/Sospechoso) + URL" .-> DASH
        DASH -- "9. Petición GET directa" --> AZ
        AZ -. "10. Descarga JSON con telemetría" .-> DASH
    end

    %% Estilos profesionales
    classDef worker fill:#ffcccc,stroke:#cc0000,stroke-width:2px;
    classDef redis fill:#ff9999,stroke:#cc0000,stroke-width:2px;
    classDef engine fill:#e6ccff,stroke:#6600cc,stroke-width:2px;
    classDef azure fill:#cce5ff,stroke:#0066cc,stroke-width:2px;
    classDef postgres fill:#99ccff,stroke:#005ce6,stroke-width:2px;
    classDef front fill:#e6ffcc,stroke:#009900,stroke-width:2px;
